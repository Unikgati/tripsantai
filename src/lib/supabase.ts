...
export async function createInvoiceForOrder(orderId: number, total: number, metadata: any = {}): Promise<any> {
  ...
  // generate a short URL-safe token for public sharing using crypto when available
  const makeToken = (len = 12) => {
    const chars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_';
    try {
      // Prefer Web Crypto API for better entropy
      const buf = typeof crypto !== 'undefined' && (crypto as any).getRandomValues
        ? (crypto as any).getRandomValues(new Uint8Array(len))
        : null;
      if (buf) {
        let t = '';
        for (let i = 0; i < buf.length; i++) t += chars[buf[i] % chars.length];
        return t;
      }
    } catch (e) {
  ...
  const payload = {
    ...
    total: total,
    metadata: metadata,
    share_token: makeToken(12),
    // Do not set created_at here; let the DB default it when inserting
  } as any;

  if (!useSupabase) {
    // Return a fallback invoice-like object that uses the orderId as the invoice id
    // so InvoicePage can resolve the order when Supabase isn't configured.
    return { id: orderId, ...payload };
  }

  try {
    const supabase = getSupabaseClient();
    // Try a straight insert first. If a unique violation occurs (an invoice already exists for this order),
    // fetch and return the existing row instead of overwriting share_token/created_at.
    ...
    // If insert failed because the row exists, fetch existing row.
    // This avoids overwriting share_token or created_at via upsert.
    ...
    // If both insert and fetch failed, fall back to returning a local invoice-like object.
